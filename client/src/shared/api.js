import { SERVER_ADDRESS, GIPHY_API_KEY, METHOD_ENUM } from "./constants";
    ...
    return {
      ...
      getAll: async function (...args) {
        try {
          ...
          const response = await (
            await fetch(url, options(METHOD_ENUM.GET))
          ).json();
      ...
      create: async function (data, ...args) {
        try {
          const url = this.formatUrl(args);
          const response = await (
            await fetch(url, options(METHOD_ENUM.POST, data))
          ).json();
      ...
      update: async function (id, ...args) {
        try {
          ...
          const response = await (
            await fetch(url, options(METHOD_ENUM.PUT, data))
          ).json();
          return response[module.resourceName];
        } catch (error) {
      ...
      delete: async function (id, ...args) {
        try {
          ...
          const url = `${basePath}/${id}`;
          const response = await (
            await fetch(url, options(METHOD_ENUM.DELETE))
          ).json();
          return response[module.resourceName];
        } catch (error) {
          throw error;
...
const fetchGifs = async (searchQuery = "") => {
  try {
    const response = await fetch(
      `https://api.giphy.com/v1/gifs/search?api_key=${GIPHY_API_KEY}&q=${searchQuery}&limit=6`,
    );
