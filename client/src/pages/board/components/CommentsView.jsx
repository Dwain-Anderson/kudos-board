...
export default function CommentsView({ card, showModal, setShowModal }) {
    ...
    const { updateCardList, boardId, closeCommentsModal } = useContext(CardListContext);
                ...
                <div className="modal-section">
                    <div className="preview-container">
                        ...
                        <div className="preview-content">
                            <p>{card.message}</p>
                            {card.author && <p className="preview-subtitle">By: {card.author}</p>}
                        </div>
                    </div>
                </div>
                        ...
                        {comments.length > 0 ? (
                            comments.map((comment, index) => (
                                <div key={index} className="list-item">
                                    <p>{comment.message}</p>
                                    {comment.author && <p className="list-item-subtitle">By: {comment.author}</p>}
                                </div>
                            ))
                        ) : (
                            <p>No comments yet.</p>
